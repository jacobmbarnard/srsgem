...
class TestAdd < Test::Unit::TestCase
  ...
  def test_match_multiple_complex_get_substrings
    section_markdown_tag = SRSSectionMarkdownTag.new
    results = section_markdown_tag.matches_for_srs_section_markdown_tag_regex("A B C  [jae g](srs:sctn) 123 [a-g as](srs:sctn) &amp;")
  ...
  def test_match_single_and_convert
    section_markdown_tag = SRSSectionMarkdownTag.new
    results = section_markdown_tag.replace_section_md_with_html("[abc](srs:sctn)")
    assert_true(results.eql? "<a href=\"\#abc\">abc</a>")
  end

  def test_match_multiple_complex_and_convert
    section_markdown_tag = SRSSectionMarkdownTag.new
    results = section_markdown_tag.replace_section_md_with_html("A B C  [jae g](srs:sctn) 123 [a-g as](srs:sctn) &amp;")
